...
class SpiderAgent():
    ...
    def sync_job_status(self, project):
        for spider_service_instance in self.spider_service_instances:
            ...
            # set status to CANCELED for running jobs that are not in scrapyd
            all_execution_info = (job_status[SpiderStatus.FINISHED]
                                  + job_status[SpiderStatus.PENDING]
                                  + job_status[SpiderStatus.RUNNING])

            not_runnning_existing_jobs = (
                job for job in job_execution_list if
                job.running_status ==SpiderStatus.RUNNING
                and job not in all_execution_info
            )

            for job_execution in not_runnning_existing_jobs:
